...
const logger = createAxiomSafeLogger('novita-internal-client');
...
class CircuitBreaker {
  ...
  async execute<T>(operation: () => Promise<T>): Promise<T> {
    if (this.state === CircuitState.OPEN) {
      if (Date.now() - this.lastFailureTime > this.config.recoveryTimeout) {
        ...
        this.successCount = 0;
        logger.info('Internal API circuit breaker transitioning to HALF_OPEN state');
      } else {
        throw new Error('Internal API circuit breaker is OPEN - requests are being rejected');
      }
    }
  ...
  private onSuccess(): void {
    ...
    if (this.state === CircuitState.HALF_OPEN) {
      ...
      if (this.successCount >= 3) {
        ...
        logger.info('Internal API circuit breaker transitioning to CLOSED state');
      }
    }
  }
  ...
  private onFailure(): void {
    ...
    if (this.failureCount >= this.config.failureThreshold) {
      ...
      logger.warn(`Internal API circuit breaker transitioning to OPEN state after ${this.failureCount} failures`);
    }
  }
...
class RateLimiter {
  ...
  async waitForSlot(): Promise<void> {
    ...
    if (this.requests.length >= this.config.maxRequests) {
      ...
      if (waitTime > 0) {
        logger.debug(`Internal API rate limit reached, waiting ${waitTime}ms`);
...
class NovitaInternalClient {
  ...
  constructor() {
    ...
    this.rateLimiter = new RateLimiter({
      maxRequests: 50, // More conservative for internal API
  ...
  private createAxiosInstance(): AxiosInstance {
    return axios.create({
      baseURL: config.novita.internalBaseUrl,
      timeout: config.defaults.requestTimeout,
      headers: {
        'Content-Type': 'application/json',
        'Authorization': `Bearer ${config.novita.internalApiKey}`,
  ...
  private setupInterceptors(): void {
    ...
    this.client.interceptors.request.use(
      (config) => {
        ...
        logger.debug('Outgoing internal API request', {
          correlationId,
      ...
      (error) => {
        logger.error('Internal API request interceptor error', { error: error.message });
    ...
    this.client.interceptors.response.use(
      (response) => {
        ...
        logger.debug('Incoming internal API response', {
          correlationId,
      ...
      async (error: AxiosError) => {
        ...
        logger.error('Internal API response error', {
          correlationId,
  ...
  private generateCorrelationId(): string {
    return `internal_req_${Date.now()}_${Math.random().toString(36).substr(2, 9)}`;
  }
  ...
  private async retryRequest(requestConfig: AxiosRequestConfig): Promise<AxiosResponse> {
    ...
    logger.info(`Retrying internal API request (attempt ${retryCount}/${config.defaults.maxRetryAttempts}) after ${delay}ms`, {
  ...
  async healthCheck(): Promise<boolean> {
    try {
      ...
      logger.warn('Novita internal API health check failed', { error: (error as Error).message });
      return false;
    }
  }
...
export const novitaInternalClient = new NovitaInternalClient();
export { CircuitState };